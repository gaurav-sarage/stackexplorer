...
import { findSubItem, findTagColor, lowercaseFirstLetter } from '@/lib/utils';
...
export default function SubItemPage({
  ...
  const triggerCount =
    1 +
    (item.overview.pricing && item.overview.pricing.length > 1 ? 1 : 0) +
    (item.resources && Object.keys(item.resources).length > 1 ? 1 : 0);

  // Determine if the tabs should be displayed (more than one trigger)
  const showTabs = triggerCount > 1;

  return (
    <div className="flex flex-col flex-1">
      <div className="flex items-center gap-2 text-sm font-semibold text-gray-500 py-4">
        <Link
          className="hover:underline"
          href={`/category/${params.categoryId}/${params.subCategoryId}`}
        >
      ...
      <div className="flex flex-col space-y-4 mb-4">
        <div className="flex flex-col space-x-0 items-start md:flex-row md:space-x-4">
          ...
          <div className="flex flex-col max-w-3xl">
            ...
            <span className="text-slate-600">{item.overview.description}</span>
          </div>
        </div>
      ...
      {showTabs && (
        <Tabs defaultValue="overview">
          <TabsList className={`grid w-full grid-cols-3`}>
            ...
            {item.resources.news &&
              Object.keys(item.resources.news).length > 0 && (
      ...
      {!showTabs && (
        // Render the content directly without tabs if there is only one trigger
        <Overview overview={item.overview} />
      )}
    </div>
  );
